    ...
    for config_idx in range(5):
        ...
        for dataset_name, dataset_path in datasets.items():
            try:
                ...
                trainer = pl.Trainer(accelerator="cpu", max_epochs=15, callbacks=[JSONLogger(metrics)], gradient_clip_val=0.5, accumulate_grad_batches=2, log_every_n_steps=1)

                print(f"[{dataset_name}] Train size: {len(train_dataset)}, Val size: {len(val_dataset)}")

                trainer.fit(model, train_loader, val_loader)
                metrics["epochs"] = trainer.current_epoch
                save_results("LSTM", dataset_name, config_idx, metrics)
