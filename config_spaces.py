...
#---- LSTM ----#
def get_lstm_config_space(seed=3):
...
#---- Transformer ----#
def get_transformer_config_space(seed=5):

    cs = ConfigurationSpace(name="Transformer", seed=seed)

    num_heads = Integer(name="num_heads", bounds=(1, 8))

    hidden_units = Integer(name="hidden_units", bounds=(32, 512), log=True)     # hidden units with logarithmic scale

    ff_dim = Integer(name="ff_dim", bounds=(128, 2048), log=True)               # feedforward dimension with logarithmic scale
